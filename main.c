...
void blur(unsigned int altura, unsigned short int pixel[512][512][3], unsigned int largura) {
  int tamanho = 0;
  scanf("%d", &tamanho);
  for (unsigned int i = 0; i < altura; ++i) {
    for (unsigned int j = 0; j < largura; ++j) {
      ...
      int menor_altura = (altura - 1 > i + tamanho/2) ? i + tamanho/2 : altura - 1;
      int min_largura = (largura - 1 > j + tamanho/2) ? j + tamanho/2 : largura - 1;
...
Imagem cortarImagem(Imagem img) {
  Imagem cortada;
...
int main() {
    ...
    for(int i = 0; i < numOpcoes; ++i) {
      ...
      switch(opcao) {
        ...
        case 3: { // Blur
          blur(img.altura, img.pixel, img.largura);
          break;
        }
        case 4: { // Rotacao
